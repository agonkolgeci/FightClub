package fr.jielos.fightclub.components;
...
public class ScoreboardSign {
    ...
    public ScoreboardSign(Player player, Scoreboards scoreboard) {
        this.player = player;
        this.scoreboard = scoreboard;
        this.objectiveName = scoreboard.getName();
    }
    ...
    public void create() {
        ...
        int i1 = 0;
        while (i1 < lines.length) sendLine(i1++);
        
        created = true;
        
        int i2 = scoreboard.getLines().length;
		for(String line : Lists.reverse(Arrays.asList(scoreboard.getLines()))) {
			setLine(i2, line);
			i2--;
		}
		
        if(!Main.getInstance().getScoreboards().containsKey(this.player)) {
        	Main.getInstance().getScoreboards().put(this.player, this);
        }
    }
    ...
    public void destroy() {
        ...
        created = false;
        if(Main.getInstance().getScoreboards().containsKey(this.player)) {
        	Main.getInstance().getScoreboards().remove(this.player);
        }
    }
